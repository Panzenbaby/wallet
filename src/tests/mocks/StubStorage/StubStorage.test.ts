...
describe("StubStorage", () => {
	...
	it("should return set and get an entry into storage", async () => {
		...
		await expect(stubStorage.get("item")).resolves.toStrictEqual("bleh");
	});
	...
	it("should check if the storage has a key", async () => {
		...
		await expect(stubStorage.has("item")).resolves.toStrictEqual(true);
	});
	...
	it("should forget a key", async () => {
		...
		await expect(stubStorage.has("item")).resolves.toStrictEqual(false);
	});
	...
	it("should return count", async () => {
		await expect(stubStorage.count()).resolves.toStrictEqual(0);
	});

	it("should restore", async () => {
		await expect(stubStorage.restore()).resolves.toStrictEqual(undefined);
	});
	it("should snapshot", async () => {
		await expect(stubStorage.snapshot()).resolves.toStrictEqual(undefined);
	});
});
