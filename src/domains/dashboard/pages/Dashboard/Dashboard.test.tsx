...
describe("Dashboard", () => {
	...
	it("should navigate to import ledger page", async () => {
		...
		expect(history.location.pathname).toStrictEqual(`/profiles/${fixtureProfileId}/wallets/import`);
		expect(asFragment()).toMatchSnapshot();
	...
	it("should navigate to create wallet page", async () => {
		...
		expect(history.location.pathname).toStrictEqual(`/profiles/${fixtureProfileId}/wallets/create`);
		expect(asFragment()).toMatchSnapshot();
	});
